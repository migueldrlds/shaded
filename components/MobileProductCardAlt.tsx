        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              {getAvailableColors().slice(0, 4).map((colorInfo, index) => (
                <div
                  key={index}
                  className={`relative w-8 h-8 rounded-full border-2 border-white/70 overflow-hidden ${!colorInfo.disponible ? 'opacity-40' : ''
                    }`}
                >
                  <Image
          ...
          <div className="flex gap-1.5 flex-wrap">
            {getAvailableSizes().slice(0, 5).map((sizeInfo, index) => (
              <span
                key={index}
                className={`inline-flex items-center justify-center px-2 py-1 rounded-full text-xs font-medium transition-all ${sizeInfo.disponible
